...
{//input x,y,z - output translate matrix
...
vector < vector<float> >  FiexedToMatrix(vector < vector<float> > input)
{//input 6 number position and Fixed angle
    ...
    {
        vector <float> uni(16);
        vector < vector<float> > inter_1;
        vector < vector<float> > inter_2;
        vector < vector<float> > inter_3;
        float x = input[i][0];
        float y = input[i][1];
        float z = input[i][2];
        float alpha = input[i][3]; //angle of rotation about x axis
        float beta  = input[i][4]; //angle of rotation about y axis
        float theta = input[i][5]; //angle of rotation about z axis
        ///T(-) Rx(-a)Ry(-b)Rz()Ry(b)Rx(a) T
        //1. Ry(b) * Rx(a)
        inter_1 = MatMuliti(Rotation_Y(beta),Rotation_X(alpha));
        //2. Rz()  * step 1
        inter_2 = MatMuliti(Rotation_Z(theta),inter_1);
        //3. T * step 2
        inter_3 = MatMuliti(Translation(x,y,z),inter_2);

        ///Row major to column major operation
        for(int j =0; j<4;j++)
        {
            for(int k =0; k<4;k++)
            {
                uni[j+k*4] = inter_3[j][k];
            }
        }
...
{//input tangent vector// output tangent rotation matrix
    ...
    vector <float> U_vector = Normalize(cross_product3D(up_vector,N_vector));
...
{//input dimention //outputidentidy matrix
